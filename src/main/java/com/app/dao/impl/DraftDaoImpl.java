...
public class DraftDaoImpl implements DraftDao {
	...
	public List<User> buscarTodos(int idTorneo) {
		...
		String query = " select  " 
				...
				+ " where prestamos.activo = 1 and prestamos.Torneos_idTorneo="+idTorneo;
	...
	public void crearPrestamo(User jugador, long id,int idTorneo) {
		System.out.println("Crear prestamo call createPrestamo(" + jugador.getId() + "," + id + ",);"+idTorneo);
		//ULtimo parametro opcion para eliminar(2) o crear(1)

		String query = "call fifaxgamersbd.createPrestamo(?,?,?,1)";

		jdbcTemplate.update(query, jugador.getId(), id,idTorneo);
	...
	public List<JugadorDraft> findJugadoresDraft(int idTorneo) {
		...
		String query = " select persona.idPersona, " 
				...
				+ " join torneos on torneos.idTorneo = draftpc.Torneos_idTorneo " 
				+ " where draftpc.abierto = 1 and torneos.idTorneo = " + idTorneo;
	...
	public HashMap<String, String> initialDraft(long id, int monto, String manager, String observaciones,
		...
		SqlParameter idTorneoOferta = new SqlParameter("idTorneo", Types.VARCHAR);
		...
		SqlParameter[] paramArray = { idJugador, montoOferta, manager1, observaciones1, idEquipoOferta,idTorneoOferta, isError,
				message };
		...
		Map storedProcResult = myStoredProcedure.execute(id, monto, manager, observaciones, idEquipo,idTorneo);
	...
	public HashMap<String, String> updateDraft(long id, int monto, String manager, String observaciones,
			int montoInicial, int idEquipo, int idTorneo) {
		...
		System.out.println("------->[updateDraft]\t id:" +id +"\t montoInicial:" +montoInicial +"\t monto:" +monto +"\t manager:" +manager +"\t observaciones:" +observaciones +"\t idEquipo:" +idEquipo +"\t idTorneo:" +idTorneo);
		SqlParameter[] paramArray = { idJugador, montoInicial1, montoOferta, manager1, observaciones1, idEquipoOferta,idTorneoOferta,
				isError, message };
		...
		// Call stored procedure
		Map storedProcResult = myStoredProcedure.execute(id, montoInicial, monto, manager, observaciones, idEquipo,idTorneo);
		System.out.println(storedProcResult);
	...
	public List<JugadorDraft> findJugadoresDraftByIdEquipo(int idEquipo, int idTorneo) {
		...
		String query = " select persona.idPersona,  " 
				...
				+ " join persona on persona.idPersona = draftpc.Persona_idPersona  "
				+ " join equipos on equipos.idEquipo = persona.Equipos_idEquipo  "
				+ " join torneos on torneos.idTorneo = draftpc.Torneos_idTorneo  " + " where draftpc.abierto = 1 "
				+ " and draftpc.Torneos_idTorneo = " + idTorneo
				+ " and draftpc.Persona_idPersona in (select DraftPC_Persona_idPersona from historicodraft "
				+ " where historicodraft.Torneos_idTorneo = " + idTorneo
	...
	@Override
	public HashMap<String, String> confirmPlayer(long id, int idEquipo, int idTorneo) {
		...
		SqlParameter[] paramArray = { idJugador, idEquipoOferta,idTorneoOferta,
				isError, message };
		...
		// Call stored procedure
		Map storedProcResult = myStoredProcedure.execute(id, idEquipo,idTorneo);
